...
export default async function handler(req, res) {
  ...
  try {
    foundEmployee = await meatingsModel.findById(req.query.id);
    if (!foundEmployee) {
      res.status(404).json({
        success: false,
        message: "Meating Not Found!",
      });
      return;
    }
  } catch (error) {
    res.status(400).json({
      success: false,
      message: "Invalid Meating Id sent!",
    });
    return;
  }
  ...
  try {
    switch (req.method) {
      case "GET":
        ...
        break;
      case "PUT":
        await meatingsModel.findByIdAndUpdate(
          req.query.id,
          { $set: req.body },
          { new: true }
        );
        res.status(201).json({
          success: true,
          message: "Meating Updated Successfully!",
        });
        break;
      case "DELETE":
        await meatingsModel.findByIdAndDelete(req.query.id);
        res.status(201).json({
          success: true,
