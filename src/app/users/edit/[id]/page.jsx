...
const Page = ({ params }) => {
  ...
  const updateItem = async (e) => {
    ...
    try {
      ...
      var res = await axios.put(`/api/users/${params.id}`, formData);
      if (res.data.success) {
    ...
    <div>
      ...
      >
        ...
        <div className="grid grid-cols-2 p-4 gap-6">
          ...
          <div className={`relative bg-white col-span-1`}>
            <input
              required
              id="email"
                ...
                >
                  <div className="border-b px-4 py-2 bg-gray-100">
                    <input
                      type="checkbox"
                      onChange={changeHandler}
                      checked={formData.rights.find(r => r.resource == v.resource)}
                      className="mr-2"
                      value={v.resource}
          ...
          <div className="col-span-2 flex justify-end">
            <button
              type="button"
              onClick={() => getSingleItem(params.id)}
            ...
            <button className="bg-blue-600 border-blue-600 border hover:bg-blue-500 px-5 py-2 rounded-md text-white">
              <span>{formLoading ? "Processing..." : "Save"}</span>
            </button>
          </div>
        </div>
      </form>
